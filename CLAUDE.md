...
data/{Title_Slug}/
  ...
  {paper_id}.pdf.txt           # extracted PDF text used by Phase 2
  {paper_id}.interpret.md      # English interpretation report (Phase 2 default)
  {paper_id}.brief.md          # article-style English brief (Phase 2 default)
  {paper_id}.interpret.json    # English structured metadata/content (Phase 2)
  {paper_id}.interpret.zh.md   # Chinese report when --cn/--trans is used
  {paper_id}.brief.zh.md       # Chinese brief when --cn/--trans is used
  {paper_id}.interpret.zh.json # Chinese structured metadata/content (Phase 2)
  {paper_id}.interpret.html    # styled English standalone HTML (Phase 3)
  {paper_id}.brief.html        # styled English brief HTML (Phase 3)
  {paper_id}.interpret.zh.html # styled Chinese standalone HTML (Phase 3)
  {paper_id}.brief.zh.html     # styled Chinese brief HTML (Phase 3)
  images/                      # extracted PDF images (if pymupdf4llm used)
    *.png
  {paper_id}.poster.en.svg       # English one-shot SVG poster (Phase 4 default)
  {paper_id}.poster.zh.svg       # Chinese one-shot SVG poster (with --cn/--trans)
  {paper_id}.poster.en.png       # SVG rendered to PNG (Phase 4 default)
  {paper_id}.poster.zh.png       # Chinese SVG rendered to PNG (with --cn/--trans)
  {paper_id}.poster.direct.en.png # English direct text-to-image poster (Phase 4 default)
  {paper_id}.poster.direct.zh.png # Chinese direct text-to-image poster (with --cn/--trans)
```
...
2. **Interpret** (`02_interpret.md`): Extracts PDF to Markdown via pymupdf4llm
   (primary) or pdftotext (fallback), saves `{paper_id}.pdf.txt`, extracts embedded
   images, selects a representative figure, validates PDF content via LLM, and
   generates English `.interpret.md` and `.brief.md` by default. `--cn` adds Chinese
   `.zh` outputs; `--trans` derives Chinese outputs from the English reports plus
   shared PDF context. DB status → `interpreted` on Phase 2 success.
3. **Convert** (`03_convert.md`): `md_to_html.py` converts English and available Chinese Markdown reports to standalone HTML with dark/light mode CSS.
4. **Poster** (`04_poster.md`): `generate_poster.py` generates 3 English poster files by default, or 6 bilingual poster files with `--cn`/`--trans`. Uses the configured text LLM + qwen-image-2.0-pro. Optional — skipped if `LLM_API_KEY` is not set.
